...
const Signup = () => {
	...
	const emailCheckMutation = useMutation({
		mutationFn: () => {
			return axios.post(
				`${BASE_URL}/api/members/send-email/${signupInfo.email}`,
			);
		},
	});

	const handleEmailCheck: MouseEventHandler<HTMLButtonElement> = async (
		event,
	) => {
	...
	const submitMutation = useMutation({
		mutationFn: () => {
			return axios.post(`${BASE_URL}/api/members/join`, formData);
		},
	});
						...
						<form onSubmit={handleSubmit}>
							<div className="flex flex-col gap-8">
								...
								<div>
									...
									/>
									<div
										className={sendEmail ? sendEmailStyle : notSendEmailStyle}
									>
										{sendEmail && (
											<p className="text-sm text-POINT_COLOR">
												작성한 이메일의 메일함 속 링크를 클릭해주세요.
											</p>
										)}
										<button
											onClick={handleEmailCheck}
											className="px-2 py-1 rounded-md bg-BTN_COLOR text-BASIC_WHITE"
										>
											이메일 인증하기
										</button>
									</div>
